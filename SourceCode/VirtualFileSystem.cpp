...
int GetFDFromName(char *name)
{
    ...
    while(i < 50)
    {
        if(UFDTArr[i].ptrfiletable != NULL)
                if(strcmp((UFDTArr[i].ptrfiletaable->ptrinode->FileName),name)==0)
                        break;
        i++;
    }
...
int CreateFile(char *name, int permission)
{
    ...
    UFDTArr[i].ptrfiletable->ptrinode->permssion = permission;
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->readoffset !=READ && UFDTArr[fd].ptrfiletable->ptrinode>FileActualSize)    return -3;
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    strncpy(arr,(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->writeoffset),arr,isize);
...
int OpenFile(char *name,int mode)
{
    ...
    UFDTArr[fd].ptrfiletable = (PTRFILETABLE)malloc(sizeof(FILETABLE));
    if(UFDTArr[fd].ptrfiletable- == NULL)   return -1;
    ...
    while(1)
    {
        ...
        else if(count == 3)
        {
            ...
            else if (strcmp(command[0],"read") == 0)
            {
                ...
                if(ptr == NULL)
                {
                    printf("Error : Memory allocation fal=ilure\n");
                    continue;
                }
